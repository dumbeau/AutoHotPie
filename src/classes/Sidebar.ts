class SidebarComponent {
    constructor(){
        ...
        this.addNewProfileButton.on('click', () => {
            Pages.newProfile.createNewProfile().then((applicationProfile) => {                
                AHPSettings.applicationProfiles.push(applicationProfile);
                Pages.editProfile.open();
                this.refresh();
            }).catch(err => {
                // console.log(err);
                Pages.editProfile.open();
    ...
    collapseButton = $('#toggle-sidebar-btn');
    collapseIconLeft = $('#sidebar-collapse-icon-left');
    collapseIconRight = $('#sidebar-collapse-icon-right');
    addNewProfileButton = $('#add-new-profile-btn');  
    sidebarApplicationProfileDiv = $('#sidebar-application-profiles-div'); 
    profiles: SidebarProfileElement[] = [];
    ...
    toggle(){        
        ...
        setTimeout(() => {  Pages.editPieMenu.pieMenuDisplay.resizeCanvas(); }, 100);
        
    };
    refresh(){ 
        this.sidebarApplicationProfileDiv.empty();       
        this.profiles = [];      
        AHPSettings.applicationProfiles.forEach( (profile) => {
...
class SidebarProfileElement {
    constructor(applicationProfile:ApplicationProfile){
    ...
    icon: string;
    name: string;
    clipboardSidebarProfileDiv = $('#clipboard-sidebar-profile-div').clone();
    element: any;
    iconDiv = this.clipboardSidebarProfileDiv.find('[name=sidebar-profile-icon-div]')[0]    
    text = this.clipboardSidebarProfileDiv.find('h5')[0]
    deleteButton = this.clipboardSidebarProfileDiv.find('.minimized-sidebar-delete-div')[0]
    ...
    buildElement(){
        ...
        if(this.icon.endsWith(".exe")){  
            ...
            // Preload.processIcon.get(this.icon).then( (fileIcon) => {            
            ...
            function createPieIcon(selectionColor: string, backgroundColor: string){
        ...
        this.element.on('click', () => {
            Pages.editProfile.loadProfile(this.applicationProfile);
        })
    }
};