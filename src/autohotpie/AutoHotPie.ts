...
class PieMenu {
    ...
    theme: PieTheme = new PieTheme({}, this);
...
class PieTheme {
  constructor(params: DeepPartial<PieTheme> = {}, _pieMenu?: PieMenu){
    _.merge(this, params);
  }
  backgroundColor: Color = new Color([35,35,35,255]);    
  selectionColor: Color = new Color([28,212,220,255]);
  ...
  labelRadius: number = 77;
  labelRoundness: number = 100;
  labelLayout: "normal" | "sundial" = "normal";
  pieAngle: number = 0;
  showSliceHotkeys: boolean = true;
}
...
class Hotkey {
    ...
    bind(callback: () => void){
      globalShortcut.register(this.accelerator, () => {
        callback();        
        console.log("ran hotkey " + this.accelerator);
      });
    }
    ...
    #refreshHotkeyData(){
        ...
        let acceleratorMods = "";
        acceleratorMods = (this.isWin) ? acceleratorMods + "Meta+" : acceleratorMods;
        acceleratorMods = (this.isShift) ? acceleratorMods + "Shift+" : acceleratorMods;
        acceleratorMods = (this.isCtrl) ? acceleratorMods + "CmdOrCtrl+" : acceleratorMods;
        acceleratorMods = (this.isAlt) ? acceleratorMods + "Alt+" : acceleratorMods;
        this.accelerator = acceleratorMods + keyListObj.displayKey;
  ...
  if (testObject.version){
      ...
      let defaultAutoHotPieSettings = new AutoHotPie({
          applicationProfiles: [new ApplicationProfile({
              ...
              executables: [new ApplicationExecutable("AHPDefaultProfile")],
...
var AHPSettings: AutoHotPie = loadAutoHotPie(settingsHandler.load());