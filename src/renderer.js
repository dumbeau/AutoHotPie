...
SettingsFileName.replace('.','-old.')
var AutoHotPieSettings;
// JSONFile.initialize();
{
try{
    AutoHotPieSettings = JSONFile.open(SettingsFileName)
    JSONFile.save(SettingsFileName.replace('.','-old.'),AutoHotPieSettings) 

    //Update old settings file to new version if relevant
    if(AutoHotPieSettings.global.functionConfig != createDefaultAHPSettings().global.functionConfig){
        AutoHotPieSettings.global.functionConfig = createDefaultAHPSettings().global.functionConfig;
    } 
    if(AutoHotPieSettings.global.htmlAhkKeyConversionTable != createDefaultAHPSettings().global.htmlAhkKeyConversionTable){
        AutoHotPieSettings.global.htmlAhkKeyConversionTable = createDefaultAHPSettings().global.htmlAhkKeyConversionTable;
    }  
}catch(e){
    AutoHotPieSettings = createDefaultAHPSettings();
...
class AHPSettings {
    constructor(){
        //Check for first pie menu, then check for default pie menu, otherwise use initial
        //update functionConfig and htmlConversion table if they don't match the current pattern
        this.global = {
